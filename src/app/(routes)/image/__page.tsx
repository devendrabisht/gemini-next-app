...
/* ---------------- Types ---------------- */
type FileWithId = File & { id: string; preview: string };
...
function SortableFileItem({
  ...
  return (
    ...
    >
      ...
      {/* File name */}
      <div className="mt-2 text-sm text-center truncate">{file.name}</div>
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd}>
          <SortableContext items={files.map((f) => f.id)} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 mt-4">
              {files.map((file) => (
                <SortableFileItem key={file.id} file={file} onDelete={handleDelete} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
      ...
      <Card className="flex items-center justify-center p-6">
        {resultImage ? (
          ...
          <div className="text-muted-foreground">Result image will appear here</div>
        )}
      </Card>
    </div>
  );
